...
if(isset($_POST['add_product'])){
    ...
    $faculty_images_2 = $_FILES["faculty_images_2"]['name'];
        ...
        $query = "INSERT INTO Waffle_product (title,price,product_image,descriptions,modal_images)
        ...
        if($query_run){
            ...
            header('location: Waffle.product.php?alert=success');
        }else{
            header('location: Waffle.product.php?error=didntsubmit');
        }
    }  
    ...
    if(isset($_POST['updatebtn'])){
        ...
        $edit_faculty_images_2 = $_FILES["edit_faculty_images_2"]['name']; 
            ...
            $query = "UPDATE waffle_product SET 
            title='$edit_title', 
            price='$edit_price',
            ...
            if($query_run){
                    ...
                    header('location: Waffle.product.php?alert=dataupdated');
              
            }else{
               
                header('location:  Waffle.product.php?alert=didntdataupdated');
            }
        
    }
    if(isset($_POST['delete_btn'])){
        $id = $_POST['delete_id'];
        $query = "DELETE  FROM waffle_product WHERE id ='$id'";
        $query_run = mysqli_query($connection,$query);
        if($query_run){
            header('location:  Waffle.product.php?alert=datadeleted');
           
        }else{
          
            header('location:  Waffle.product.php?alert=error');
        }
    }
